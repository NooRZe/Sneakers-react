...
function App() {
    ...
    */
    async function fetchData() {
      const cartResponse = await axios.get(
        "https://680b7472d5075a76d98b2cd7.mockapi.io/cart"
      );
      const favoritesResponse = await axios.get(
        "https://68136c1e129f6313e2113452.mockapi.io/favorites"
      );
      const itemsResponse = await axios.get(
        "https://680b7472d5075a76d98b2cd7.mockapi.io/items"
      );
  ...
  const onAddToCart = (obj) => {
    ...
    if (cartItems.find((cartObj) => Number(cartObj.id) === Number(obj.id))) {
      axios.delete(
        `https://680b7472d5075a76d98b2cd7.mockapi.io/cart/${obj.id}`
      );
      setCartItems((prev) =>
      ...
      axios
        .post("https://680b7472d5075a76d98b2cd7.mockapi.io/cart", obj)
  ...
  const onAddToFavorites = (obj) => {
    if (favorites.find((favObj) => favObj.id === obj.id)) {
      axios.delete(
        `https://68136c1e129f6313e2113452.mockapi.io/favorites/${obj.id}`
      );
      setFavorites((prev) => prev.filter((item) => item.id !== obj.id));
    } else {
      axios
        .post("https://68136c1e129f6313e2113452.mockapi.io/favorites", obj)
